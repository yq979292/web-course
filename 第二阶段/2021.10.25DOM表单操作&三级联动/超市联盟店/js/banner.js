function Slide(container, options, callback, obj){
	...
	this.list=$(container+' .banner_list');
	this.handle=$(container+' .Num li');
	this.item=$(container+' .banner_list li');
	this.itemWH=0;
...
Slide.prototype={
	init:function(){
		...
		if(op.effect==='slide'){
			...
			this.container.find('.banner_slide_clip').css({position:'relative',overflow:'hidden',height:itemH,width:itemW});
